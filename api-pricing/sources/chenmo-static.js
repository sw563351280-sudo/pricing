/**
 * chenmo-static.js — 沉默西林 API 中转站 (静态数据)
 * 更新方式: 登录后从 Network 导出 pricing 请求的 Response → data/chenmo-raw.json
 */
const fs = require("fs");
...
async function fetch() {
  const rawPath = path.join(__dirname, "..", "data", "chenmo-raw.json");
  if (!fs.existsSync(rawPath)) throw new Error("chenmo-raw.json not found");
  ...
  return { source: "chenmoai.cn", display_name: "沉默西林", base_currency: "USD", _missing_group_ratios: [...missing].sort(), models };
}
