/**
 * dongli-static.js — 冻梨 API 中转站 (静态数据)
 * 更新方式: 登录后从 Network 导出 pricing 请求的 Response → data/dongli-raw.json
 */
const fs = require("fs");
...
async function fetch() {
  const rawPath = path.join(__dirname, "..", "data", "dongli-raw.json");
  if (!fs.existsSync(rawPath)) throw new Error("dongli-raw.json not found");
  ...
  return { source: "dongli.work", display_name: "冻梨", base_currency: "USD", _missing_group_ratios: [...missing].sort(), models };
}
